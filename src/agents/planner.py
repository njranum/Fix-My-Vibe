...
def _generate_security_md(scan_result: dict, kb_context: dict | None = None) -> str:
    """Generate SECURITY.md from code-level findings — the AI code audit report.

    When kb_context is supplied (Foundry mode — it's fetched from Azure AI Search
    per finding type), a "References" section grounds the guidance in the KB
    sources. This is deterministic: it renders on EVERY run with findings, so the
    Foundry IQ grounding is always visible, independent of whether the LLM
    remediator happened to emit a cited in-place patch.
    """
    ...
    # KB-grounded references — deterministic proof of Foundry IQ on every run.
    if kb_context:
        ref_lines: list[str] = []
        seen_types: list[str] = []
        ...
        for ft in seen_types:
            ...
            for r in kb_context.get(ft, []):
                ...
                if len(titles) >= 2:
                    break
            if titles:
                label = ft.replace("_", " ").capitalize()
                ref_lines.append(f"- **{label}** — {'; '.join(titles)}")
        if ref_lines:
            lines.append("## References")
            lines.append("")
            lines.append("Guidance above is grounded in these knowledge-base sources "
                         "(OWASP / CWE / NIST, via Azure AI Search):")
...
def _normalize_ranks(plan: dict) -> None:
    ...
    # Keep plan_summary in sync with the final action set — actions may have been
    # added (remediations) or removed since run() first wrote it, so a stale count
    # here is what made the plan say "4 actions" while returning 5.
    high = sum(1 for a in actions if a.get("priority") == "high")
    med = sum(1 for a in actions if a.get("priority") == "medium")
    summary = f"{len(actions)} action(s) required."
    if high:
        summary += f" {high} high priority."
    if med:
        summary += f" {med} medium priority."
...
def _ensure_security_actions(plan: dict, scan_result: dict) -> None:
    ...
    one run dropped SECURITY.md + .gitignore despite 7 findings and an exposed
    .env). Security actions are non-negotiable, so if the parsed plan is missing
    them, inject the locally-generated versions.
    """
    ...
    if injected:
        ...
        plan["plan_summary"] = (
            ...
            + f" [{len(injected)} action(s) restored by deterministic backstop.]"
        ).strip()
